          ...
          <tr v-for="(item, index) in basket" :key="'basket' + item.id">
            ...
            <td>
              <div class="basket-item__quantity">
                <button class="quantity-button" v-on:click="decreaseItemQuantity(item)">–</button>
                <input type="number" :value="item.quantity" min="1" inputmode="numeric" />
                <button class="quantity-button" v-on:click="increaseItemQuantity(item)">+</button>
              </div>
            </td>
            <td>
              <p class="basket-item__price">${{ calcSum[index] }} </p>
            </td>
            <td>
              <button class="btn btn-delete" aria-label="Удалить" v-on:click="removeItem(index)">
                <svg
